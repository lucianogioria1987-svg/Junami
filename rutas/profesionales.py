...
def profesionales():
    ...
    for p in lista:
        esp = p.get('especialidad', p.get('rol', ''))
        areas.setdefault(esp, []).append(p)
    ...
    if request.method == 'POST':
        ...
        lista.append({
            "id": nid, "nombre": request.form['nombre'], "usuario": request.form['usuario'], "contrasena": request.form['contrasena'], "rol": request.form['rol'], "especialidad": request.form.get('especialidad', 'Terapia Ocupacional'),
...
def editar_profesional(id):
    ...
    for p in lista:
        if p['id'] == id:
            ...
            p['especialidad'] = request.form.get('especialidad', 'Terapia Ocupacional')
...
@profesionales_bp.route('/profesional/eliminar/<int:id>', methods=['POST'])
def eliminar_profesional(id):
    if 'usuario_actual' not in session: return redirect(url_for('dashboard_loguin_bp.login'))
    lista = cargar_datos('profesionales.json')
    
    # Prevenir auto-eliminación si es necesario, o simplemente eliminar
    if session['usuario_actual'].get('id') == id:
        # Se podría enviar un mensaje flash, pero redirigimos por ahora
        return redirect(url_for('profesionales'))

    lista_nueva = [p for p in lista if p['id'] != id]
    if len(lista_nueva) < len(lista):
        with open(os.path.join('datos', 'profesionales.json'), 'w', encoding='utf-8') as f: 
            json.dump(lista_nueva, f, indent=4, ensure_ascii=False)
